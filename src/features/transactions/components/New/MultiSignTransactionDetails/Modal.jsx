...
import { normalizeGlobalEYAmount , converIntToDec } from 'utility/buildInOutDisplay'
...
class Modal extends React.Component {
  ...
  normalize(inouts) {
    ...
    inouts.forEach(inout => {
      ...
      if(filteredAsset.length === 1){
        ...
        amount = assetId===eyID?
          normalizeGlobalEYAmount(eyID, amount, this.props.eyAmountUnit):
  ...
  render() {
    ...
    let view = <div></div>
    if (item.length !== 0) {
      const fee = normalizeGlobalEYAmount(eyID, item.fee, eyAmountUnit)
