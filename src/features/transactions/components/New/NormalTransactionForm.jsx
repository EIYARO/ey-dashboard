...
class NormalTxForm extends React.Component {
  ...
  confirmedTransaction(e, assetDecimal){
    ...
    this.props.showModal(
      <ConfirmModal
        ...
        eyAmountUnit={this.props.eyAmountUnit}
  ...
  estimateNormalTransactionGas(type) {
    ...
    const isEY = (assetAlias==='EY') || (assetId === eyID)
    ...
    if(isEY && isChainTx){
      ...
      this.connection.request('/build-chain-transactions', body).then(resp => {
        ...
        const errorMsg =  err.code && i18n.exists(`eyError.${err.code}`) && t(`eyError.${err.code}`) || err.msg
  ...
  render() {
    ...
    const availableBalance = balance(this.props.values, assetDecimal, this.props.balances, this.props.eyAmountUnit)

    const showEyAmountUnit = (assetAlias.value === 'EY' || assetId.value === eyID)
    ...
    >
      <div className={styles.borderBottom}>
        ...
        {showEyAmountUnit && [<label className={styles.title}>{t('transaction.normal.submitType')}</label>,
