  ...
  render() {
    ...
    Object.keys(summary).forEach((assetId) => {
      ...
      Object.keys(asset).forEach((accountId) => {
        ...
        const assetAlias = asset.alias ==='EY'? this.props.eyAmountUnit: asset.alias
        if (accountId !== 'external') {
          if(account['spend']> account['control'] && account['spend'] > 0){
            ...
            if(asset.retire === 0 ){
              ...
              items.push({
                type: 'sent',
                amount: normalizeEyAmountUnit(assetId, gas, this.props.eyAmountUnit),
            ...
            items.push({
              type: type,
              amount: asset.decimals? converIntToDec(amount, asset.decimals) : normalizeEyAmountUnit(assetId, amount, this.props.eyAmountUnit),
              asset: assetAlias ? assetAlias : <code className={styles.rawId}>{assetId}</code>,
              assetId: assetId,
              account: account.alias ? account.alias : <code className={styles.rawId}>{accountId}</code>,
              accountId: accountId,
