    ...
    if (item) {
      ...
      view = <div className={componentClassNames(this)}>
        <PageTitle
          title={title}
          actions={[
            item.id!==eyID && <Link key='create-asset-btn' className='btn btn-link' to={`/transactions/create?type=issueAsset&alias=${item.alias}`}>{t('transaction.issue.issueAsset')}</Link>,
          ]}
        />
        ...
        <PageContent>
          <KeyValueTable
            ...
            items={[
              ...
              {label: t('form.alias'), value: item.alias, editUrl: item.alias === 'EY' ? null : `/assets/${item.id}/alias`},
              {label: t('form.symbol'), value: item.definition.symbol},
              {label: t('form.decimals'), value: item.definition.decimals},
              {label: t('form.reissueTitle'), value:  (item.alias === 'EY' || item.limitHeight > 0)? 'false ': 'true', note: assetLabel},
